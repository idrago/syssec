...
// Custom logger
const logStream = fs.createWriteStream(path.join(logsDir, 'vulnerable-api.log'), { flags: 'a' });
app.use(morgan('combined', { stream: logStream }));
app.use(morgan('dev')); // Console logging

// VULNERABLE: Allow all origins (misconfigured CORS)
app.use((req, res, next) => {
  const origin = req.get('Origin');

  // If there is an Origin header, echo it back
  if (origin) {
...
app.use((req, res, next) => {
  ...
  console.log(`[DEBUG] Session Cookie:`, req.cookies.teacherSession);
  console.log('---');
  next();
});
...
app.post('/api/login', (req, res) => {
  ...
  if (teachers[teacherId] && password === 'password123') {
    ...
    // VULNERABLE: HTTPS and sameSite: 'none' for CSRF demo
    res.cookie('teacherSession', sessionId, { 
      httpOnly: false, // VULNERABLE: JavaScript can access
      secure: true,    // Required for HTTPS and sameSite: 'none'
      sameSite: 'none' // VULNERABLE: Allows cross-site requests
    });
    
    console.log(`[VULNERABLE-UNIVERSITY-API] Teacher login successful for ${teacherId}, session: ${sessionId}`);
    console.log(`[DEBUG] Cookie set with secure: true, sameSite: none`);
...
app.post('/api/change-grade', (req, res) => {
  ...
  console.log(`[DEBUG] Change grade request - Session ID: ${sessionId}`);
  ...
  if (grades[targetStudentId] && grades[targetStudentId][course]) {
    ...
    console.log(`[VULNERABLE-UNIVERSITY-API] GRADE CHANGE: ${sessionData.userId} (${sessionData.userType}) changed ${targetStudentId}'s ${course}: ${oldGrade} -> ${newGrade}`);
    console.log(`[VULNERABLE-UNIVERSITY-API] Origin: ${req.headers.origin || 'No Origin'}`);
    console.log(`[VULNERABLE-UNIVERSITY-API] Referer: ${req.headers.referer || 'No Referer'}`);
...
app.post('/api/register-course', (req, res) => {
  ...
  if (students[targetStudentId] && !students[targetStudentId].courses.includes(courseCode)) {
    ...
    console.log(`[VULNERABLE-UNIVERSITY-API] COURSE REGISTRATION: ${sessionData.userId} (${sessionData.userType}) registered ${targetStudentId} for ${courseCode}`);
    console.log(`[VULNERABLE-UNIVERSITY-API] Origin: ${req.headers.origin || 'No Origin'}`);
...
app.get('/api/status', (req, res) => {
  res.json({ 
    ...
    cors: 'misconfigured (*)',
    csrf: 'none',
    cookies: 'sameSite: none + secure (vulnerable to CSRF)',
    https: 'enabled with shared CA',
    warning: 'This portal is intentionally vulnerable to CSRF',
    endpoints: [
      'POST /api/login',
      'POST /api/change-grade (VULNERABLE)',
      'POST /api/register-course (VULNERABLE)', 
      'GET /api/students',
      'GET /api/courses',
      'GET /api/profile',
      'GET /api/grades',
...
try {
  ...
  server.listen(PORT, '0.0.0.0', () => {
    console.log(`Vulnerable API running on HTTPS port ${PORT}`);
    console.log(`Access at: https://localhost:${PORT}`);
    console.log('Security: DISABLED - This portal is intentionally vulnerable to CSRF');
    console.log('SSL: Using shared Certificate Authority');
    console.log('Demo accounts:');
    console.log('  Teachers: teacher123/password123');
    console.log('');
    console.log('CSRF Configuration: sameSite=none + secure=true (vulnerable)');
    ...
    console.log('  POST /api/change-grade - VULNERABLE to CSRF');
    console.log('  POST /api/register-course - VULNERABLE to CSRF');
  });
